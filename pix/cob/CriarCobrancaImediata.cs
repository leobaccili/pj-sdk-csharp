	...
	public class CriarCobrancaImediata {
		public CobrancaDetalhada Criar(Config config, Cobranca cobranca) {
			...
			if (cobranca.Txid == null ) {
				json = HttpUtils.CallPost(config, url, Constants.ESCOPO_PIX_COBRANCA_WRITE, "Erro ao criar cobrança imediata", json);
			} else {
				url = url + "/" + cobranca.Txid;
				json = HttpUtils.CallPut(config, url, Constants.ESCOPO_PIX_COBRANCA_WRITE, "Erro ao criar cobrança imediata", json);
			}
