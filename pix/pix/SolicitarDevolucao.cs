...
namespace Sdk.PixApi {
	public class SolicitarDevolucao {
		public DevolucaoDetalhada Solicitar(Config config, string e2eId, string id, Devolucao devolucao) {
			...
			string json = SdkUtils.Serialize(devolucao);
			json = HttpUtils.CallPut(config, url, Constants.ESCOPO_PIX_WRITE, "Erro ao solicitar devolução", json);
